...
export type AIProvider = 'openai' | 'claude' | 'gemini' | 'deepseek' | 'grok' | 'mistral';
export const AI_PROVIDER_LIST: AIProvider[] = ['openai', 'claude', 'gemini', 'deepseek', 'grok', 'mistral'];
...
async function getUserKey(userId: number, provider: AIProvider): Promise<string | null> {
  const row = await queryOne<{ api_key_enc: string }>(
    'SELECT api_key_enc FROM user_ai_keys WHERE user_id=? AND provider=? AND is_active=1',
    [userId, provider]
  );
  if (!row) return null;
  try { return decrypt(row.api_key_enc); } catch { return null; }
}
...
export async function runAI(
  ...
  let result: string;
  try {
    result = await callProvider(provider, apiKey, prompt);
  } catch (err: any) {
  ...
  await query(
    ...
    [userId, provider, PROVIDER_CONFIG[provider].model, action, usedOwnKey ? 1 : 0]
  );
